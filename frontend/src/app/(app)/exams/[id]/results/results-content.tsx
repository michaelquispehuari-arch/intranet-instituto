"use client";

import Link from "next/link";
import { useTranslation } from "@/lib/i18n/LanguageContext";
import { INTL_LOCALES } from "@/lib/i18n/types";
import type { ExamResults } from "../../types";

function formatScore(value: number | null) {
  return value === null ? "–" : value.toFixed(2);
}

type ResultsContentProps = {
  examId: string;
  role: "ADMIN" | "PROFESOR" | "ESTUDIANTE";
  studentId?: string;
  notAvailable: boolean;
  beforeCloseAt: string | null;
  data: ExamResults | null;
};

export function ResultsContent({ examId, role, studentId, notAvailable, beforeCloseAt, data }: ResultsContentProps) {
  const { t, locale } = useTranslation();

  const cierreLabel = beforeCloseAt
    ? new Intl.DateTimeFormat(INTL_LOCALES[locale], {
        timeZone: "America/Lima",
        hour: "2-digit",
        minute: "2-digit",
        day: "2-digit",
        month: "short",
      }).format(new Date(beforeCloseAt))
    : null;

  const backLink = (
    <div style={{ marginBottom: 12 }}>
      <Link href="/exams" style={{ fontSize: 13, color: "var(--texto-tenue)" }}>
        {t("exams.backToExamsLink")}
      </Link>
    </div>
  );

  if (notAvailable) {
    return (
      <div>
        {backLink}
        <div className="card">
          <div className="empty-state">
            <div className="empty-state-icon">⏳</div>
            <p className="empty-state-title">{t("exams.resultsNotAvailableTitle")}</p>
            <p>{t("exams.resultsNotAvailableDesc")}</p>
          </div>
        </div>
      </div>
    );
  }

  if (cierreLabel) {
    return (
      <div>
        {backLink}
        <div className="card">
          <div className="empty-state">
            <div className="empty-state-icon">⏳</div>
            <p className="empty-state-title">{t("exams.resultPendingTitle")}</p>
            <p>{t("exams.resultPendingDesc", { hora: cierreLabel })}</p>
          </div>
        </div>
      </div>
    );
  }

  if (!data || data.submissions.length === 0) {
    return (
      <div>
        {backLink}
        <div className="page-header">
          <span className="page-eyebrow">{t("exams.resultsEyebrow")}</span>
          <h1 className="page-title">{data?.exam.titulo ?? t("exams.examFallbackTitle")}</h1>
        </div>
        <div className="card">
          <div className="empty-state">
            <div className="empty-state-icon">📊</div>
            <p className="empty-state-title">{t("exams.noSubmissionsTitle")}</p>
            <p>{t("exams.noSubmissionsDesc")}</p>
          </div>
        </div>
      </div>
    );
  }

  // ADMIN / PROFESOR: lista de alumnos → detalle por studentId
  if (role === "ADMIN" || role === "PROFESOR") {
    if (studentId) {
      const sub = data.submissions.find((s) => s.estudiante.id === studentId);
      if (!sub) {
        return (
          <div>
            {backLink}
            <div style={{ marginBottom: 12 }}>
              <Link href={`/exams/${examId}/results`} style={{ fontSize: 13, color: "var(--texto-tenue)" }}>
                {t("exams.backToStudentList")}
              </Link>
            </div>
            <div className="card">
              <div className="empty-state">
                <p className="empty-state-title">{t("exams.studentNotFoundTitle")}</p>
                <p>{t("exams.studentNotFoundDesc")}</p>
              </div>
            </div>
          </div>
        );
      }

      return (
        <div>
          {backLink}
          <div style={{ marginBottom: 12 }}>
            <Link href={`/exams/${examId}/results`} style={{ fontSize: 13, color: "var(--texto-tenue)" }}>
              {t("exams.backToStudentList")}
            </Link>
          </div>
          <div className="page-header">
            <span className="page-eyebrow">{t("exams.answersEyebrow")}</span>
            <h1 className="page-title">{data.exam.titulo}</h1>
            <p className="page-subtitle">
              {sub.estudiante.nombre} {sub.estudiante.apellido} · {sub.estudiante.email}
            </p>
          </div>
          <section className="card" style={{ padding: 20 }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 16 }}>
              <span className="chip chip-ok">{t("exams.totalScore", { puntaje: formatScore(sub.puntajeTotal) })}</span>
              {sub.enviadoEn && (
                <span style={{ fontSize: 13, color: "var(--texto-tenue)" }}>
                  {t("exams.submittedAtColon", {
                    fecha: new Intl.DateTimeFormat(INTL_LOCALES[locale], {
                      dateStyle: "medium",
                      timeStyle: "short",
                      timeZone: "America/Lima",
                    }).format(new Date(sub.enviadoEn)),
                  })}
                </span>
              )}
            </div>
            <div className="stack compact-stack">
              {sub.respuestas.map((answer) => (
                <div className="answer-row" key={answer.id}>
                  <strong>{answer.pregunta.texto}</strong>
                  <p>{t("exams.answerValue", { respuesta: answer.respuesta })}</p>
                  {answer.estadoCalificacion === "PENDIENTE" ? (
                    <p style={{ color: "var(--amber-action)" }}>{t("exams.pendingManualGrading")}</p>
                  ) : answer.esCorrecta ? (
                    <p className="success-text">{t("exams.correctPoints", { puntaje: answer.puntajeObtenido })}</p>
                  ) : (
                    <p className="error">
                      {t("exams.incorrectPoints", { puntaje: answer.puntajeObtenido })}
                      {answer.pregunta.respuestaCorrecta
                        ? ` ${t("exams.correctAnswerSuffix", { respuesta: answer.pregunta.respuestaCorrecta })}`
                        : ""}
                    </p>
                  )}
                </div>
              ))}
            </div>
          </section>
        </div>
      );
    }

    // Lista de alumnos
    return (
      <div>
        {backLink}
        <div className="page-header">
          <span className="page-eyebrow">{t("exams.resultsEyebrow")}</span>
          <h1 className="page-title">{data.exam.titulo}</h1>
          <p className="page-subtitle">{t("exams.studentsCompleted", { count: data.submissions.length })}</p>
        </div>
        <div className="session-list" aria-label={t("exams.studentListAria")}>
          {data.submissions.map((sub) => {
            const aprobado = sub.puntajeTotal !== null ? sub.puntajeTotal >= 11 : null;
            return (
              <article className="session-card" style={{ cursor: "default" }} key={sub.id}>
                <div className="session-info">
                  <div className="session-title">
                    {sub.estudiante.nombre} {sub.estudiante.apellido}
                  </div>
                  <div style={{ fontSize: 13, color: "var(--texto-tenue)" }}>{sub.estudiante.email}</div>
                  <div className="session-chips" style={{ marginTop: 6 }}>
                    <span className={`chip ${aprobado === true ? "chip-ok" : aprobado === false ? "chip-resumen" : "chip-capturas"}`}>
                      {t("exams.scorePts", { puntaje: formatScore(sub.puntajeTotal) })}
                    </span>
                    {sub.enviadoEn && (
                      <span style={{ fontSize: 12, color: "var(--texto-tenue)", alignSelf: "center" }}>
                        {t("exams.submittedAtPlain", {
                          fecha: new Intl.DateTimeFormat(INTL_LOCALES[locale], {
                            dateStyle: "medium",
                            timeStyle: "short",
                            timeZone: "America/Lima",
                          }).format(new Date(sub.enviadoEn)),
                        })}
                      </span>
                    )}
                  </div>
                </div>
                <Link
                  href={`/exams/${examId}/results?studentId=${sub.estudiante.id}`}
                  className="btn btn-secondary"
                  style={{ fontSize: 13, flexShrink: 0 }}
                >
                  {t("exams.viewExam")}
                </Link>
              </article>
            );
          })}
        </div>
      </div>
    );
  }

  // ESTUDIANTE: solo ve su propio resultado
  const mySubmission = data.submissions[0];
  return (
    <div>
      {backLink}
      <div className="page-header">
        <span className="page-eyebrow">{t("exams.myResultEyebrow")}</span>
        <h1 className="page-title">{data.exam.titulo}</h1>
        <p className="page-subtitle">{t("exams.totalScore", { puntaje: formatScore(mySubmission.puntajeTotal) })}</p>
      </div>
      <section className="card" style={{ padding: 20 }}>
        <div className="stack compact-stack">
          {mySubmission.respuestas.map((answer) => (
            <div className="answer-row" key={answer.id}>
              <strong>{answer.pregunta.texto}</strong>
              <p>{t("exams.answerValue", { respuesta: answer.respuesta })}</p>
              {answer.esCorrecta ? (
                <p className="success-text">{t("exams.correctPoints", { puntaje: answer.puntajeObtenido })}</p>
              ) : (
                <p className="error">
                  {t("exams.incorrectPoints", { puntaje: answer.puntajeObtenido })}
                  {answer.pregunta.respuestaCorrecta
                    ? ` ${t("exams.correctAnswerSuffix", { respuesta: answer.pregunta.respuestaCorrecta })}`
                    : ""}
                </p>
              )}
            </div>
          ))}
        </div>
      </section>
    </div>
  );
}
